...
\(R\)的所有特征值分别为\(\mu_1,\cdots,\mu_\zeta\)，都有\(U=\oplus_{i=1}^\zeta G(\mu_i,R)\).

则当\(\mydim V=n\)时，首先有\(V=\Ker (T-\lambda_1 I)^n \oplus \Img (T-\lambda_1 I)^n\).
不妨令\(U=\Img (T-\lambda_1 I)^n\).
...
\textit{Proof}
使用数学归纳法.当\(n=1\)时，\(V=G(\lambda,T)\)，命题成立.
...
故任意给定\(v_1,\cdots,w_p\)作为\(V\)的一组基使得\(M(T|_U,(w_1,\cdots,w_p))\)是上三角矩阵，
